...
def attack( image, model=None, metric_range=100, device='cpu',
    ...
    loss_fn = lambda output,target: torch.sum(output*torch.log(target))
    ...
    for _ in range(iters):
        ...
        image = Variable(image + additive, requires_grad=True)
        output = dmodel(image)
        # print(output, target)
        ...
        additive = 0.25*eps*gradient_sign
    
    res_image = image + additive
    # r = model(res_image)
    # print("Result:", r, torch.sum(res_image-i))
